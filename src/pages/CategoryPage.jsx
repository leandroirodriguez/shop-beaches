...
export default function CategoryPage() {
  ...
  return (
    <main className="max-w-[720px] mx-auto px-5 pb-24">
      ...
      <h1 className="font-headline text-4xl text-on-surface mt-3">{category.name} Essentials</h1>
      {category.description && (
        <p className="text-on-surface-variant mt-3 leading-relaxed">{category.description}</p>
      )}
      ...
      {products.length === 0 ? (
        ...
        <div className="mt-8 space-y-6">
          {products.map(p => (
            <article
              key={p.id}
              className="rounded-xl bg-surface-container-low shadow-lift overflow-hidden relative"
            >
              ...
              <div className="p-5">
                <div className="flex items-start justify-between gap-4">
                  <Link to={`/product/${p.slug}`} className="hover:underline">
                    <h2 className="font-headline text-xl text-on-surface leading-snug">{p.display_title}</h2>
                  </Link>
                  {p.amazon_price && <p className="font-headline text-lg text-primary shrink-0">{p.amazon_price}</p>}
                </div>
                ...
                {p.provider_note && (
                  <div className="mt-4 p-4 rounded-md bg-surface-container">
                    ...
                    <p className="italic text-sm text-on-surface-variant mt-1 leading-relaxed">
                      "{p.provider_note}"
                    </p>
                  </div>
                )}
                ...
                <Link
                  to={`/product/${p.slug}`}
                  className="mt-4 w-full inline-flex items-center justify-center gap-2 px-5 py-3 rounded-md bg-primary text-on-primary font-label text-sm tracking-wider shadow-lift hover:bg-primary-container transition"
                >
                  View Details
